...
class Solution {
public:
    void recoverData(vector<vector<int>> &sets) {
        ...
        for (i = 0; i < sets.size(); i++) {
            for (j = 0; j < sets[i].size(); j++) {
                nmap[sets[i][j]] = i;
                nset.insert(sets[i][j]);
            }
        }

        cout << endl;
        // now go through each data center and print output
        for (i = 0; i < sets.size(); i++) {
            unordered_set<int> dset;
            for (j = 0; j < sets[i].size(); j++) dset.insert(sets[i][j]);
            for ( auto it = nset.begin(); it != nset.end(); ++it) {
                if (dset.find(*it) == dset.end()) {
                    int from = nmap[*it];
                    cout << *it << " " << from+1 << " " << i+1 << endl;
                }
            }
        }
