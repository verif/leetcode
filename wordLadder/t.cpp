...
// BFS gives the best potential performance
class Solution {
public:
     vector<string> findNextWords(string last, unordered_set<string> &dict,
                                  string end) {
         vector<string> ret;
         for (int i = 0; i < last.length(); i++) {
             for (char c = 'a'; c <= 'z'; c++) {
                 if (c == last.at(i)) continue;
                 string copy = last;
                 copy = copy.replace(i, 1, 1, c);
                 if (dict.find(copy) != dict.end()) {
                     ret.push_back(copy);
                 }
                 else if (!copy.compare(end)) {
                     ret.clear();
                     ret.push_back(copy);
                     return ret;
                 }
     ...
     vector<vector<string>> findLadders(string start, string end,
         ...
         vvs.push_back(vs);
         dict.erase(start);
         if (!start.compare(end)) return vvs;

         while (1) {
             bool found = false;
             bool foundNewWords = false;
             vector<vector<string>> vvs1;
             vector<vector<string>>::const_iterator it;
             unordered_map<string, vector<string>> nextWordMap;
             for (it = vvs.begin(); it != vvs.end(); it++) {
                 vs = *it;
                 string last = vs.back();
                 unordered_map<string, vector<string>>::iterator nit =
                     nextWordMap.find(last);
                 vector<string> nwords;
                 if (nit != nextWordMap.end()) {
                     nwords = nit->second;
                 }
                 else {
                     nwords = findNextWords(last, dict, end);
                     nextWordMap[last] = nwords;
                 }
                 vector<string>::const_iterator it2;
                 for (it2 = nwords.begin(); it2 != nwords.end(); it2++) {
                     string word = *it2;
                     if (!word.compare(end)) {
                         found = true;
                     }
                     foundNewWords = true;
                     vector<string> nvs(vs);
                     nvs.push_back(word);
                     vvs1.push_back(nvs);
                 }
             }
             curLevel++;
             vvs = vvs1;
             // Remove words in current level from dict & prune other vectors
             // if 'end' already found
             ...
             if (!foundNewWords) { // 'end' not found
                 break;
             }
         }
...
static void printLadders(vector<vector<string>>  & ret)
{
    vector<vector<string>>::const_iterator it;
